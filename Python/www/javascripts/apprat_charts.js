...
d3.csv("Test1_SM_v2.csv", function(error, experiments) {
	 ...
	 ndx = crossfilter(experiments),
      ...
      ctaGroup = ctaYearDim.group().reduceSum(function(d){ return parseFloat(+d.CTA/1000000)})
	...
	d3.select('#Country').on('change', function(){
		...
		if(con == "All"){
		 ...
		 updateTCO(countDim);
		 if(ctaYearDim.top(Infinity).length == 0){
				console.log("hello")
				ctaGroup = ctaYearDim.group().reduceSum(function(d){ return +d.CTA;})
        tcoeBarChart2.group(ctaGroup);
			}
		}
		  updateTest(alldata);
		  updateAppCount(projTcoeDim)
...
var ofs = 0, pag = 25; var end=0
  ...
  function dynamicDD(){
     ...
     document.getElementById("Country").options[10]=new Option("Netherlands");
     document.getElementById("Country").options[11]=new Option("Norway");
	 }
/*	  if(reg == "LATAM"){		
		 countryDim.filter(null)	  
	     document.getElementById("Country").options.length = 0;
